...
void recursivePrint(struct Node *p)
{
  ...
  if (p)
  {
    ...
    else if (p->left && p->right && !p->middle)
...
void case2(struct Node *p, struct Node *q, int end, int middle, int up, int x)
{
  ...
  { // If x lies between key1 and key2 of parent
    if (x < q->keys[0])
    { // if we are on the left child
      q->keys[1] = q->keys[0];
      q->keys[0] = up;
...
void case3(struct Node *p, struct Node *q, struct Node *w, int lChild, int rChild, int parent, int top, int x)
{
  ...
  newParent->right = r;
  if (q->isRoot)
    newRoot = newNode(newRoot, top, true); // We know the parent node's value because if we are on the left child, whichever key we pick will be smaller than any of the parent's keys
  else
    newRoot = newNode(newRoot, top, false);
  q->keys[0] = q->keys[1];
  q->keys[1] = -1;
  if (q->left == p)
  { // If we are on the left child
  ...
  else if (q->right == p)
  {
    ...
    q->middle = NULL;
    if (w != NULL)
    { // If our new root has a parent to attach itself to
      w->right = newRoot;
      return;
    }
    newRoot->isRoot = true;
  ...
  else if (w != NULL && q->middle == p)
  { // If we are on the middle child and there is a parent we attach our new root to
    if (w->keys[0] != -1 && w->keys[1] != -1)
    {                               // If the parent we are attaching the root to is xx
    ...
    {                               // If the parent we are attaching the root to is xo
      tempLeft = newParent->left;   // Since we have a separate node with middle elements from the previous node as children we have to preserve the sorted order
      tempRight = newParent->right; // So we assign our new parent's left value to its right node
      newParent->left = q->left;    // And its right value to the previous node's left node
      newParent->right = tempLeft;  // The right value of the new parent will be the value for the previous node's left node
  ...
  {                               // If q is the root and we are on the middle child
    tempLeft = newParent->left;   // Since we have a separate node with middle elements from the previous node as children we have to preserve the sorted order
    tempRight = newParent->right; // So we assign our new parent's left value to its right node
    newParent->left = q->left;    // And its right value to the previous node's left node
    newParent->right = tempLeft;  // The right value of the new parent will be the value for the previous node's left node
...
void iInsert(struct Node *p, int x)
{
  ...
  { // If root is full and tree is built
    while (p != NULL)
    {
      if (x == p->keys[0] || x == p->keys[1])
        return;
      if (q != NULL && q->keys[0] != -1 && q->keys[1] != -1)
      { // If parent xx we may have case 3 depending on whether p is xx or not and what child p is
        ...
        { // If current is xx
          ...
          { // x lies between first and second key
            if (!p->middle)
            { // we have to know what children it is on so we can set the new root's value
              if (q->left == p)
              {
                case3(p, q, w, p->keys[0], p->keys[1], x, q->keys[0], x);
              }
        ...
        { // If current is xo and previous is xx
          if (!p->right)
          {
            p->keys[1] = x;
            return;
